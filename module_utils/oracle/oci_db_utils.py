...
def is_version_changed(patch_history_entry_getter_method, patch_getter_method, current_version, input_version_dict, last_patch_history_entry_id, **kwargs):
    version_changed = False
    patch_details = None
    if is_patch_already_applied(patch_getter_method, current_version, input_version_dict, **kwargs):
    ...
    last_patch_history = oci_utils.call_with_backoff(patch_history_entry_getter_method,
                                                     **kwargs).data
    if input_version_dict.get('patch_id') != last_patch_history.patch_id:
        ...
        if not (last_patch_history.action == 'APPLY' and last_patch_history.lifecycle_state == 'SUCCEEDED') and \
                last_patch_history.action != input_action or last_patch_history.lifecycle_state != 'SUCCEEDED':
            patch_details = get_patch_details_from_version(
                input_version_dict)
            return True, patch_details
...
def is_patch_already_applied(patch_getter_method, current_version, input_version_dict, **kwargs):
    if input_version_dict is None:
        return True
    patch_id = input_version_dict.get('patch_id')
    kwargs.update(patch_id=patch_id)
    existing_patch = oci_utils.call_with_backoff(
        patch_getter_method, **kwargs).data
...
def get_patch_details_from_version(input_version_dict):
    ...
    for attribute in patch_details.attribute_map.keys():
        patch_details.__setattr__(
            attribute, input_version_dict.get(attribute, None))
...
def execute_function_and_wait(resource_type, client, function, kwargs_function, get_fn, get_param, module, states=None,
                              kwargs_get=None):
