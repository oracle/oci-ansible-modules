...
def list_load_balancer_hostnames(lb_client, module):
    ...
    try:
        if not name:
            get_logger().debug("Listing all Hostnames under load balancer %s", load_balancer_id)
            existing_hostnames = oci_utils.list_all_resources(lb_client.list_hostnames,
                                                              load_balancer_id=load_balancer_id)
        else:
            get_logger().debug("Listing Backend Set %s on load balancer %s", name, load_balancer_id)
            response = oci_utils.call_with_backoff(lb_client.get_hostname, load_balancer_id=load_balancer_id,
                                                   name=name)
