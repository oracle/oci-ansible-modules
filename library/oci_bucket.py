    ...
    try:
        if bucket:
            changed = should_bucket_be_changed(
                bucket, module.params['metadata'],
                module.params['public_access_type'])
            if changed:
                get_logger().info("Updating bucket %s in namespace %s",
            ...
            get_logger().info("Creating bucket %s in namespace %s",
                              bucket_name, namespace_name)
            changed = True
        ...
        get_logger().error("Failed to create or update bucket %s", bucket_name,
                           exc_info=True)
...
def get_existing_bucket(object_storage_client, module):
    ...
    try:
        ...
        if ex.status != 404:
            get_logger().error("Failed to perform checking existing bucket",
                               exc_info=True)
            module.fail_json(msg=ex.message)
        get_logger().debug(
...
def should_bucket_be_changed(existing_bucket, metadata, public_access_type):
    return existing_bucket.metadata != metadata \
...
def create_bucket(object_storage_client, module):
    ...
    return object_storage_client.create_bucket(namespace_name,
...
def update_bucket(object_storage_client, module):
    ...
    return object_storage_client.update_bucket(namespace_name, bucket_name,
...
def delete_bucket(object_storage_client, module):
    ...
    get_logger().info("Deleting bucket %s in namespace %s with force option:%s",
                      bucket_name, namespace_name, force)
    ...
    try:
        if existing_bucket:
            ...
            object_storage_client.delete_bucket(namespace_name, bucket_name,
                                                **opc_client_request_id_dict)
            changed = True
            get_logger().info("Successfully deleted bucket % s in namespace % s \
                              with force option: % s",
                              bucket_name, namespace_name, force)
    except ServiceError as ex:
        get_logger().error("Failed to delete bucket %s", bucket_name,
                           exc_info=True)
        module.fail_json(msg=ex.message)
...
def delete_all_objects_in_bucket(object_storage_client, namespace_name,
                                 bucket_name):
    get_logger().debug("Deleting all objects inside bucket: %s", bucket_name)
    fields_to_retrieve = 'name'
    objects_in_bucket = oci_utils.\
        list_all_resources(object_storage_client.list_objects,
    ...
    for obj in objects_in_bucket.objects:
        ...
        object_storage_client.delete_object(
            namespace_name, bucket_name, obj.name)
