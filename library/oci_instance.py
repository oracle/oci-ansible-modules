...
def power_action_on_instance(compute_client, id, desired_state, module):
    result = {}
    changed = False
    # The power action to execute on a compute instance to reach
    # the desired 'state'
    ...
    # The desired lifecycle state for the compute instance to
    # reach the user specified 'state'
    ...
    try:
        ...
        # We need to perform a power action if either the current
        # state doesn't match the desired state
...
def ensure_exact_count(compute_client, exact_count, count_tag, module):
    ...
    if curr_inst_count == exact_count:
        ...
        for inst in matching_instances[-to_delete:]:
            ...
            terminated_instances.append(terminated_result['instance'])

        result['changed'] = True
        result['instances'] = to_dict(matching_instances[:exact_count])
        result['terminated_instances'] = to_dict(terminated_instances)
...
def _get_next_available_suffix(matching_instances, exact_count, name_prefix):
    current_instance_names = [to_dict(inst)['display_name'] for inst in matching_instances]
    for num in range(exact_count):
        candidate_name = _generate_name_for_instance(name_prefix, num)
        if candidate_name not in current_instance_names:
            return num
