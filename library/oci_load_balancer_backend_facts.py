...
def list_load_balancer_backends(lb_client, module):
    ...
    try:
        if ip_address and port:
            ...
            get_logger().info("Listing all attributes of  backend %s for backendset %s \
                               in load balancer %s", backend_name, backend_set_name,
                              load_balancer_id)
            response = lb_client.get_backend(
                load_balancer_id, backend_set_name, backend_name)
            backends = [response.data]
        else:
            get_logger().info("Listing all attributes of  all backends of backendset %s \
