...
def list_load_balancer_backend_sets(lb_client, module):
    ...
    try:
        if not name:
            get_logger().debug("Listing all Backend Sets under \
            load balancer %s", load_balancer_id)
            existing_backend_sets = oci_utils.list_all_resources(lb_client.list_backend_sets,
                                                                 load_balancer_id=load_balancer_id)
        else:
            get_logger().debug("Listing Backend Set %s on load balancer %s", name, load_balancer_id)
            response = oci_utils.call_with_backoff(lb_client.get_backend_set, load_balancer_id=load_balancer_id,
