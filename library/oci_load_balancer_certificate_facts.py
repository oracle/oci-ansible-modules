...
def list_load_balancer_certificates(lb_client, module):
    ...
    try:
        if name is None or not name:
            get_logger().debug("Listing all certificates under load balancer %s", load_balancer_id)
            existing_certificates = oci_utils.list_all_resources(
                lb_client.list_certificates,
                load_balancer_id=load_balancer_id)
        else:
            get_logger().debug("Listing certificate %s on load balancer %s", name, load_balancer_id)
            response = oci_utils.call_with_backoff(lb_client.list_certificates, load_balancer_id=load_balancer_id)
