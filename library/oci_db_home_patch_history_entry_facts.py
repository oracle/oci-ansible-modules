...
def list_db_home_patch_history_entries(db_client, module):
    ...
    try:
        if patch_history_entry_id:
            get_logger().debug("Listing DB Home Patch History Entry for ID  %s of DB Home %s",
                               patch_history_entry_id, db_home_id)
            response = oci_utils.call_with_backoff(
                db_client.get_db_home_patch_history_entry,
                db_home_id=db_home_id, patch_history_entry_id=patch_history_entry_id)
            db_home_patch_history_entries = [response.data]
        else:
            get_logger().debug("Listing all DB Home Patch History Entries of DB Home %s",
                               db_home_id)
            db_home_patch_history_entries = oci_utils.list_all_resources(
                db_client.list_db_home_patch_history_entries,
                db_home_id=db_home_id)
    except ServiceError as ex:
        get_logger().error("Unable to list DB Home Patch History Entries due to %s", ex.message)
