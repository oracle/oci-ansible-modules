...
def list_load_balancer_work_requests(lb_client, module):
    ...
    try:
        if load_balancer_id:
            get_logger().debug("Listing all Work Requests under load balancer %s", load_balancer_id)
            existing_work_requests = oci_utils.list_all_resources(lb_client.list_work_requests,
                                                                  load_balancer_id=load_balancer_id)
        else:
            get_logger().debug("Listing Work Request %s ", work_request_id)
            response = oci_utils.call_with_backoff(lb_client.get_work_request, work_request_id=work_request_id)
