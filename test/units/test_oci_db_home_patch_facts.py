...
try:
    ...
    raise SkipTest(
        "test_oci_db_home_patch_facts.py requires `oci` module")
...
def test_list_db_home_patches_with_patch_id(db_client):
    module = get_module(dict({'db_home_id': 'ocid1.dbhome.aaaa', 'patch_id': 'ocid1.patch.aaaa'}))
    db_client.get_db_home_patch.return_value = get_response(200, None, get_patch_versions(), None)
    result = oci_db_home_patch_facts.list_db_home_patches(db_client, module)
    assert result['db_home_patches'][0]['version'] is get_patch_versions().version
