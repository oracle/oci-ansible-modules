...
def test_create_or_update_db_home_create(db_client, get_existing_resource_patch, check_and_create_resource_patch):
    ...
    check_and_create_resource_patch.return_value = {'db_home': to_dict(db_home), 'changed': True}
...
def test_create_or_update_db_home_client_error(db_client, check_and_create_resource_patch):
...
def test_create_or_update_db_home_service_error(db_client, check_and_create_resource_patch):
...
def test_create_db_home_source_db_backup(db_client, create_and_wait_patch):
    module = get_module(dict({"database" : {
...
def test_update_db_home_version_changed(db_client, update_db_home_patch, oci_db_utils_is_version_changed_patch):
    module = get_module(dict())
    db_home = get_db_home()
...
def test_delete_db_home(db_client, delete_and_wait_patch):
    module = get_module(
        dict({'db_home_id': 'ocid1.dbhome.aaa'}))
    db_home = get_db_home()
...
def get_module(additional_properties):
    params = {
        ...
        "database": {"admin_password": "BEstr0ng_#1", "character_set": "AL32UTF8",
