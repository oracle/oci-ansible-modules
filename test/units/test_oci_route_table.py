...
def test_create_or_update_route_table_create(virtual_network_client, check_and_create_resource_patch):
    ...
    check_and_create_resource_patch.return_value = dict(changed=True, route_table=route_table)
    result = oci_route_table.create_or_update_route_table(
        virtual_network_client, module)
    assert result['changed'] is True


def test_create_or_update_route_table_update(virtual_network_client, update_route_table_patch, get_existing_resource_patch):
    route_table = get_route_table()
...
def test_create_or_update_route_service_error(virtual_network_client, check_and_create_resource_patch):
    error_message = 'Internal Server Error'
    module = get_module(None)
    check_and_create_resource_patch.side_effect = ServiceError(
        499, 'InternalServerError', dict(), error_message)
    try:
        oci_route_table.create_or_update_route_table(
            virtual_network_client, module)
    except Exception as ex:
        assert error_message in ex.args[0]


def test_create_or_update_route_timeout_error(virtual_network_client, check_and_create_resource_patch):
...
def test_create_route_table(virtual_network_client, create_and_wait_patch):
    route_table = get_route_table()
    route_rules = [{'cidr_block': '0.0.0.0/0', 'network_entity_id': '10.0.10.5'},
                   {'cidr_block': '10.0.0.4/16', 'network_entity_id': '10.0.20.5'}]
    module = get_module(dict({'route_rules': route_rules}))
    create_and_wait_patch.return_value = dict(changed=True, route_table=to_dict(route_table))
...
def test_update_route_table_defined_tags_changed(virtual_network_client, update_and_wait_patch):
    route_table = get_route_table()
    module = get_module(dict(defined_tags=dict(admin_type=dict(admin_role='limited_user'))))
    update_and_wait_patch.return_value = dict(changed=True, route_table=route_table)
    result = oci_route_table.update_route_table(virtual_network_client, route_table, module)
    assert result['changed'] is True
...
def test_update_route_table_empty_input_route_rules(virtual_network_client, update_and_wait_patch):
    route_table = get_route_table()
    route_rules = []
    module = get_module(dict(route_rules=route_rules))
    update_and_wait_patch.return_value = dict(changed=True, route_table=to_dict(route_table))
...
def test_delete_route_table(virtual_network_client, delete_and_wait_patch):
    ...
    delete_and_wait_patch.return_value = dict(changed=True, route_table=to_dict(route_table))
    result = oci_route_table.delete_route_table(virtual_network_client, module)
    assert result['changed'] is True


def test_delete_route_table_rt_not_exists(virtual_network_client, delete_and_wait_patch):
    module = get_module(dict(rt_id='ocid.routetable..xvdz'))
