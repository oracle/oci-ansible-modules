...
def test_list_user_groups_failure_service_error(identity_client, list_all_resources_patch):
...
def test_list_user_groups_failure_timeout_error(identity_client, list_all_resources_patch):
...
def test_get_all_groups(identity_client, list_all_resources_patch, get_associated_users):
    list_all_resources_patch.return_value = get_group_list()
    get_associated_users.return_value = to_dict(get_user())
    result = oci_group_facts.get_all_groups(
        identity_client, 'ocid1.tenancy.phx.xdfgyz')
...
def test_get_associated_users(identity_client, get_existing_group_members):
    ...
    identity_client.get_user.return_value = get_response(
        200, None, get_user(), None)
...
def test_get_associated_users_pass_ignore_non_existing_user(identity_client, get_existing_group_members):
    group_ids = ['ocid1.group.oc1.phx.xccdx8e', 'ocid1.group.oc1.phx.xpssf5r']
    get_existing_group_members.return_value = group_ids
    identity_client.get_user.side_effect = ServiceError(
        404, 'NotFound', dict(), 'User not present'), get_response(
        200, None, get_user(), None)
    result = oci_group_facts.get_associated_users(
        identity_client, 'ocid1.tenancy.phx.xdfgyz', 'ocid1.group.oc1.phx.xccdx8e')
    for member in result['members']:
...
def test_get_associated_users_raise_service_error(identity_client, get_existing_group_members):
    group_ids = ['ocid1.group.oc1.phx.xccdx8e', 'ocid1.group.oc1.phx.xpssf5r']
    get_existing_group_members.return_value = group_ids
    identity_client.get_user.side_effect = ServiceError(
