...
try:
    ...
    raise SkipTest(
        "test_oci_autonomous_database_backup_facts.py requires `oci` module")
...
def test_list_autonomous_database_backups_list_all(db_client, list_all_resources_patch):
    module = get_module(dict(compartment_id='ocid1.compartment.aaaa', autonomous_database_backup_id='ocid1.autonomousdatabasebackup.aaaa'))
    list_all_resources_patch.return_value = get_autonomous_database_backups()
    result = oci_autonomous_database_backup_facts.list_autonomous_database_backups(db_client, module)
    assert len(result['autonomous_database_backups']) is 2

def test_list_autonomous_database_backups_list_specific(db_client):
    module = get_module(dict({'autonomous_database_backup_id': 'ocid1.autonomousdatabasebackup.aaaa'}))
    ...
    result = oci_autonomous_database_backup_facts.list_autonomous_database_backups(db_client, module)
    assert result['autonomous_database_backups'][0]['display_name'] is autonomous_database_backup.display_name
...
def test_list_autonomous_database_backup_service_error(db_client):
    ...
    try:
        oci_autonomous_database_backup_facts.list_autonomous_database_backups(db_client, module)
