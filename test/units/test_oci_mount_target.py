...
try:
    ...
    raise SkipTest(
        "test_oci_mount_target.py requires `oci` module")
...
def test_create_or_update_mount_target_create(file_storage_client, check_and_create_resource_patch):
    module = get_module()
    mount_target = get_mount_target()
    check_and_create_resource_patch.return_value = {
        'mount_target': to_dict(mount_target), 'changed': True}
...
def test_create_or_update_mount_target_update(file_storage_client, update_mount_target_patch):
...
def test_create_or_update_mount_targete_client_error(file_storage_client, check_and_create_resource_patch):
...
def test_create_or_update_mount_targete_service_error(file_storage_client, check_and_create_resource_patch):
...
def test_create_mount_target(file_storage_client, create_and_wait_patch):
    module = get_module()
    mount_target = get_mount_target()
    create_and_wait_patch.return_value = {'mount_target': to_dict(mount_target), 'changed': True}
    result = oci_mount_target.create_mount_target(file_storage_client, module)
    assert result['mount_target']['display_name'] is mount_target.display_name


def test_update_mount_target_display_name(file_storage_client, check_and_update_resource_patch):
    mount_target = get_mount_target()
...
def test_update_mount_target_freeform_tags(file_storage_client, check_and_update_resource_patch):
    mount_target = get_mount_target()
    module = get_module(dict(freeform_tags=dict(deploy_type='dev'),
...
def test_update_mount_target_defined_tags(file_storage_client, check_and_update_resource_patch):
    mount_target = get_mount_target()
    module = get_module(dict(defined_tags=dict(plan=dict(corporate='medium')),
                             mount_target_id='ocid1.mounttarget.aaa'))
    check_and_update_resource_patch.return_value = {
        'mount_target': to_dict(mount_target), 'changed': True}
    result = oci_mount_target.update_mount_target(file_storage_client, module)
...
def test_delete_mount_target(file_storage_client, delete_and_wait_patch):
    module = get_module(dict(mount_target_id='ocid1.mounttarget.aaa'))
    mount_target = get_mount_target()
    delete_and_wait_patch.return_value = {'mount_target': to_dict(mount_target), 'changed': True}
