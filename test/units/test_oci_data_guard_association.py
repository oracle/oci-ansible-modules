...
try:
    ...
    raise SkipTest(
        "test_oci_db_home.py requires `oci` module")
...
def test_create_data_guard_association(db_client, create_and_wait_patch):
    ...
    create_and_wait_patch.return_value = {
        'data_guard_association': to_dict(data_guard_association), 'changed': True}
    result = oci_data_guard_association.create_data_guard_association(
        db_client, module)
...
def test_create_data_guard_association_service_error(db_client):
    ...
    try:
        oci_data_guard_association.create_data_guard_association(
            db_client, module)
    except Exception as ex:
        assert error_message in ex.args[0]


def test_perform_data_guard_operations_switchover(db_client, get_existing_resource_patch, execute_function_and_wait_patch):
    ...
    execute_function_and_wait_patch.return_value = {
        'data_guard_association': to_dict(data_guard_association), 'changed': True}
    result = oci_data_guard_association.perform_data_guard_operations(
        db_client, module)
    assert result['changed'] is True


def test_perform_data_guard_operations_switchover_on_standby_role(db_client, get_existing_resource_patch):
    module = get_module(dict({'state': 'switchover'}))
...
def test_perform_data_guard_operations_failover(db_client, get_existing_resource_patch, execute_function_and_wait_patch):
...
def test_perform_data_guard_operations_failover_on_primary_role(db_client, get_existing_resource_patch):
    module = get_module(dict({'state': 'failover'}))
    data_guard_association = get_data_guard_association('PRIMARY', 'STANDBY')
    get_existing_resource_patch.return_value = data_guard_association
    result = oci_data_guard_association.perform_data_guard_operations(
...
def test_perform_data_guard_operations_reinstate(db_client, get_existing_resource_patch, execute_function_and_wait_patch):
    module = get_module(dict({'state': 'reinstate'}))
    data_guard_association = get_data_guard_association(
        'DISABLED_STANDBY', 'PRIMARY')
    get_existing_resource_patch.return_value = data_guard_association
    execute_function_and_wait_patch.return_value = {
        'data_guard_association': to_dict(data_guard_association), 'changed': True}
    result = oci_data_guard_association.perform_data_guard_operations(
...
def test_perform_data_guard_operations_reinstate_on_non_disable_standby_role(db_client, get_existing_resource_patch):
    module = get_module(dict({'state': 'reinstate'}))
    data_guard_association = get_data_guard_association('STANDBY', 'PRIMARY')
    get_existing_resource_patch.return_value = data_guard_association
    result = oci_data_guard_association.perform_data_guard_operations(
