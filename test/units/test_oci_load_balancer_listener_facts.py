...
try:
    import oci
    from oci.load_balancer.models import Listener, SSLConfigurationDetails, LoadBalancer, ConnectionConfiguration
    from oci.exceptions import ServiceError
except ImportError:
    raise SkipTest(
        "test_oci_load_balancer_listener_facts.py requires `oci` module")
...
def test_list_load_balancer_listeners_specific_listener(lb_client):
    module = get_module(dict({'name': 'ansible_listener',
    ...
    result = oci_load_balancer_listener_facts.list_load_balancer_listeners(lb_client, module)
