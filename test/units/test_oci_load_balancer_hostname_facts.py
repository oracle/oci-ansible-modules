...
try:
    ...
    raise SkipTest(
        "test_oci_load_balancer_hostname_facts.py requires `oci` module")
...
def test_list_load_balancer_hostnames_specific_hostname(lb_client):
    module = get_module(dict({'load_balancer_id': 'ocid1.lodbalancer.xcds',
                              'name': 'hostname_001', 'hostname': 'app.example.com'}))
    lb_client.get_hostname.return_value = get_response(200, None, get_hostname(), None)
    result = oci_load_balancer_hostname_facts.list_load_balancer_hostnames(lb_client, module)
    assert result['hostnames'][0]['name'] is get_hostname().name


def test_list_load_balancer_hostnames_all_hostnames(lb_client, list_all_resources_patch):
    module = get_module(dict({'load_balancer_id': 'ocid1.lodbalancer.xcds'}))
...
def test_list_load_balancer_hostnames_service_error(lb_client, list_all_resources_patch):
