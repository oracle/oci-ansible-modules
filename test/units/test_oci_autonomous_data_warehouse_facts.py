...
try:
    ...
    raise SkipTest(
        "test_oci_autonomous_data_warehouse_facts.py requires `oci` module")
...
def test_list_autonomous_data_warehouses_list_all(db_client, list_all_resources_patch):
    ...
    result = oci_autonomous_data_warehouse_facts.list_autonomous_data_warehouses(db_client, module)
    assert len(result['autonomous_data_warehouses']) is 2


def test_list_autonomous_data_warehouses_list_specific(db_client):
    module = get_module(dict({'autonomous_data_warehouse_id': 'ocid1.autonomousdbwarehouse.aaaa'}))
    db_client.get_autonomous_data_warehouse.return_value = get_response(
        200, None, get_autonomous_data_warehouse(), None)
    result = oci_autonomous_data_warehouse_facts.list_autonomous_data_warehouses(db_client, module)
    assert result['autonomous_data_warehouses'][0]['display_name'] is 'ansible_autonomous_data_warehouse'
...
def test_list_autonomous_data_warehouses_service_error(db_client):
    ...
    try:
        oci_autonomous_data_warehouse_facts.list_autonomous_data_warehouses(db_client, module)
