...
try:
    ...
    raise SkipTest(
        "test_oci_snapshot.py requires `oci` module")
...
def test_create_or_update_snapshot_create(file_storage_client, check_and_create_resource_patch):
    module = get_module()
    snapshot = get_snapshot()
    check_and_create_resource_patch.return_value = {
        'snapshot': to_dict(snapshot), 'changed': True}
...
def test_create_or_update_snapshote_client_error(file_storage_client, check_and_create_resource_patch):
...
def test_create_or_update_snapshote_service_error(file_storage_client, check_and_create_resource_patch):
...
def test_create_snapshot(file_storage_client, create_and_wait_patch):
    module = get_module()
    snapshot = get_snapshot()
    create_and_wait_patch.return_value = {'snapshot': to_dict(snapshot), 'changed': True}
    result = oci_snapshot.create_snapshot(file_storage_client, module)
    assert result['snapshot']['name'] is snapshot.name


def test_update_snapshot_freeform_tags(file_storage_client, check_and_update_resource_patch):
    snapshot = get_snapshot()
...
def test_update_snapshot_defined_tags(file_storage_client, check_and_update_resource_patch):
    snapshot = get_snapshot()
...
def test_delete_snapshot(file_storage_client, delete_and_wait_patch):
    ...
    snapshot = get_snapshot()
    delete_and_wait_patch.return_value = {'snapshot': to_dict(snapshot), 'changed': True}
