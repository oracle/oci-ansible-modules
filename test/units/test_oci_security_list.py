...
try:
    ...
    from oci.core.models import CreateSecurityListDetails, SecurityList, UpdateSecurityListDetails, \
...
def test_create_or_update_security_list_create(virtual_network_client, check_and_create_resource_patch):
...
def test_create_or_update_security_list_update(virtual_network_client, update_security_list_patch, get_existing_resource_patch):
    security_list = get_security_list(None, None, 'ansible_security_list')
...
def test_create_or_update_security_list_service_error(virtual_network_client, check_and_create_resource_patch):
...
def test_create_security_list(virtual_network_client, create_and_wait_patch):
    ...
    egress_security_rules = [{"destination": "0.0.0.0/0", "protocol": "17",
                              "udp_options": {"source_port_range": {"max": 22, "min": 22}, "destination_port_range": None}}]
    module = get_module(dict({'egress_security_rules': egress_security_rules,
    ...
    create_and_wait_patch.return_value = dict({'security_list': to_dict(security_list), 'changed': True})
...
def test_delete_security_list(virtual_network_client, delete_and_wait_patch):
    ...
    security_list = get_security_list(None, None, 'ansible_security_list')
    delete_and_wait_patch.return_value = dict({'security_list': to_dict(security_list), 'changed': True})
...
def get_security_rules(security_rule_type, security_rule_flavour, tcp_source, icmp_source, udp_source,
                       stateless, tcp_destination, udp_destination, icmp_destination, icmp_type, icmp_code):
    security_rules = []
    ...
    if security_rule_type == 'ingress':
        if security_rule_flavour == 'hashed':
            security_rule_tcp = oci_utils.create_hashed_instance(
                IngressSecurityRule)
            ...
            tcp_destination_port_range = oci_utils.create_hashed_instance(
                PortRange)
            security_rule_udp = oci_utils.create_hashed_instance(
                IngressSecurityRule)
            ...
            udp_destination_port_range = oci_utils.create_hashed_instance(
                PortRange)
            security_rule_icmp = oci_utils.create_hashed_instance(
                IngressSecurityRule)
