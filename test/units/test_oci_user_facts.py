...
def get_group_ids_from_existing_memberships(mocker):
    return mocker.patch.object(oci_user_facts, 'get_group_ids_from_existing_memberships')
...
def test_list_users_failure_service_error(identity_client, list_all_resources_patch):
    ...
    try:
        oci_user_facts.list_users(
            identity_client, get_module(dict()))
    except Exception as ex:
        assert error_message in ex.args[0]


def test_get_all_users(identity_client, get_associated_groups, list_all_resources_patch):
...
def test_get_associated_groups_success(identity_client, get_group_ids_from_existing_memberships):
    group = get_group()
    existing_memberships_group_ids = ['ocid1.group.oc1.phx.xccdx8e']
    get_group_ids_from_existing_memberships.return_value = existing_memberships_group_ids
...
def test_get_associated_groups_ignore_non_existing_groups(identity_client, get_group_ids_from_existing_memberships):
    group_two = get_group()
    group_two.name = 'Group Two'
    existing_memberships_group_ids = [
        'ocid1.group.oc1.phx.xccdx8e', 'ocid1.group.oc1.phx.axdfg']
    get_group_ids_from_existing_memberships.return_value = existing_memberships_group_ids
    identity_client.get_group.side_effect = [ServiceError(
        404, 'NotFound', dict(), 'Group not present'), get_response(200, None, group_two, None)]
    result = oci_user_facts.get_associated_groups(
        identity_client, 'test_compartment', 'ocid1.user.oc1..xdvf')
    assert result['member_of_groups'][0]['name'] is group_two.name


def test_get_associated_groups_raise_other_error(identity_client, get_group_ids_from_existing_memberships):
    existing_memberships_group_ids = [
        'ocid1.group.oc1.phx.xccdx8e', 'ocid1.group.oc1.phx.axdfg']
    get_group_ids_from_existing_memberships.return_value = existing_memberships_group_ids
    identity_client.get_group.side_effect = ServiceError(
