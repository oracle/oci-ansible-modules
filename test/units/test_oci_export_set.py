...
try:
    ...
    raise SkipTest(
        "test_oci_export_set.py requires `oci` module")
...
def test_update_export_sete_client_error(file_storage_client, check_and_update_resource_patch):
...
def test_update_export_set_service_error(file_storage_client, check_and_update_resource_patch):
...
def test_update_export_set_display_name(file_storage_client, check_and_update_resource_patch):
    export_set = get_export_set()
    export_set.display_name = 'ansible_updated_export_set'
    module = get_module(dict({'export_set_id': 'ocid1.exportset.aaa'}))
    check_and_update_resource_patch.return_value = {
        'export_set': to_dict(export_set), 'changed': True}
    result = oci_export_set.update_export_set(file_storage_client, module)
    assert result['changed'] is True


def test_update_export_set_max_fs_stat_bytes(file_storage_client, check_and_update_resource_patch):
    export_set = get_export_set()
    module = get_module(dict(max_fs_stat_bytes=500,
                             export_set_id='ocid1.exportset.aaa'))
    check_and_update_resource_patch.return_value = {
        'export_set': to_dict(export_set), 'changed': True}
    result = oci_export_set.update_export_set(file_storage_client, module)
    assert result['changed'] is True
...
def test_update_export_set_max_fs_stat_files(file_storage_client, check_and_update_resource_patch):
    export_set = get_export_set()
    module = get_module(dict(max_fs_stat_files=500,
                             export_set_id='ocid1.exportset.aaa'))
    check_and_update_resource_patch.return_value = {
        'export_set': to_dict(export_set), 'changed': True}
    result = oci_export_set.update_export_set(file_storage_client, module)
