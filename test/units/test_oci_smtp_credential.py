...
try:
    ...
    raise SkipTest(
        "test_oci_smtp_credential.py requires `oci` module")
...
def test_create_or_update_smtp_credential_create(identity_client, check_and_create_resource_patch):
    module = get_module()
    smtp_credential = get_smtp_credential()
    check_and_create_resource_patch.return_value = {
        'smtp_credential': to_dict(smtp_credential), 'changed': True}
    result = oci_smtp_credential.create_or_update_smtp_credential(identity_client, module)
    assert result['smtp_credential']['description'] is smtp_credential.description


def test_create_or_update_smtp_credential_update(identity_client, update_smtp_credential_patch):
...
def test_create_smtp_credential(identity_client, create_and_wait_patch):
    module = get_module()
    smtp_credential = get_smtp_credential()
    create_and_wait_patch.return_value = {'smtp_credential': to_dict(smtp_credential), 'changed': True}
    result = oci_smtp_credential.create_smtp_credential(identity_client, module)
    assert result['smtp_credential']['description'] is smtp_credential.description


def test_update_smtp_credential_descriptiom(identity_client, check_and_update_resource_patch):
    smtp_credential = get_smtp_credential()
    module = get_module(dict(description='updated description',
...
def test_delete_smtp_credential(identity_client, delete_and_wait_patch):
    ...
    smtp_credential = get_smtp_credential()
    delete_and_wait_patch.return_value = {'smtp_credential': to_dict(smtp_credential), 'changed': True}
