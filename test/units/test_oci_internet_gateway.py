...
def test_create_or_update_internet_gateway_create_success(virtual_network_client, check_and_create_resource_patch):
...
def test_create_or_update_internet_gateway_update_success(virtual_network_client, update_internet_gateway_patch):
    internet_gateway = get_internet_gateway()
    module = get_module(dict({'ig_id': 'test_ig_id'}))
    get_existing_resource_patch.return_value = internet_gateway
    update_internet_gateway_patch.return_value = dict(changed=True, internet_gateway=to_dict(internet_gateway))
    result = oci_internet_gateway.create_or_update_internet_gateway(
        virtual_network_client, module)
    assert result['internet_gateway']['display_name'] == internet_gateway.display_name
    assert result['changed'] is True
...
def test_create_or_update_internet_gateway_service_error(virtual_network_client, update_internet_gateway_patch):
...
def test_create_or_update_internet_gateway_timeout_error(virtual_network_client, update_internet_gateway_patch):
...
def test_create_internet_gateway(virtual_network_client, create_and_wait_patch):
    module = get_module(
        dict({'display_name': 'ansible_ig', 'is_enabled': True}))
    internet_gateway = get_internet_gateway()
    create_and_wait_patch.return_value = dict({'internet_gateway': to_dict(internet_gateway), 'changed': True})
    result = oci_internet_gateway.create_internet_gateway(virtual_network_client, module)
    assert result['internet_gateway']['display_name'] == internet_gateway.display_name

def test_update_internet_gateway_success(virtual_network_client, check_and_update_resource_patch):
    module = get_module(
        dict(display_name='ansible_ig_updated', ig_id='ocid1.internetgateway..xvzf'))
    internet_gateway = get_internet_gateway()
    check_and_update_resource_patch.return_value = dict(
        {'internet_gateway': internet_gateway, 'changed': True})
    result = oci_internet_gateway.update_internet_gateway(virtual_network_client, module)
