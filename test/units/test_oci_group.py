...
def test_create_or_update_group_create_success(mocker, identity_client, check_and_create_resource_patch, get_existing_resource_patch):
...
def test_create_or_update_group_failure_service_error(mocker, identity_client, check_and_create_resource_patch):
...
def test_create_or_update_group_failure_timeout_error(mocker, identity_client, check_and_create_resource_patch):
...
def test_create_or_update_group_update_success(mocker, identity_client, get_existing_resource_patch):
    group = create_group_object()
    ...
    result = oci_group.create_or_update_group(
        identity_client, get_module({'compartment_id': 'test_compartment', 'group_id': 'test_group_id'}))
...
def test_create_group_without_user_success(identity_client, oci_wait_until_patch):
    ...
    group = create_group_object()
    identity_client.create_group.return_value = get_response(
        200, None, group, None)
    identity_client.get_group.return_value = get_response(
        200, None, group, None)
    result = oci_group.create_group(
...
def test_create_group_with_user_success(mocker, identity_client, oci_wait_until_patch):
    additional_properties = dict({'state': 'present', 'users': [
                                 'test_user1'], 'compartment_id': 'test_compartment'})
...
def test_update_for_group_description_changed_success(mocker, identity_client, update_group_description_and_tags_patch):
    additional_properties = dict(
        {'state': 'present', 'users': None, 'compartment_id': 'test_compartment'})
    module = get_module(additional_properties)
    group = create_group_object()
    update_group_description_and_tags_patch.return_value = group, True
    changed, _ = oci_group.update_group(
        identity_client, group, module)
    assert update_group_description_and_tags_patch.called
    assert changed is True


def test_update_user_changed_success(mocker, identity_client, oci_wait_until_patch, update_group_description_and_tags_patch, get_existing_group_members_patch):
    additional_properties = dict({'state': 'present', 'users': ['test_user']})
    mock_update_members = mocker.patch.object(
        oci_group, 'update_group_users')
    module = get_module(additional_properties)
    group = create_group_object()
...
def test_add_users_to_group_success(identity_client):
    ...
    oci_group.add_users_to_group(
        identity_client, create_group_object().id, user_ids)
    assert identity_client.add_user_to_group.called


def test_update_user_changed_no_user_success(mocker, identity_client, oci_wait_until_patch, update_group_description_and_tags_patch, get_existing_group_members_patch):
    additional_properties = dict({'state': 'present', 'users': []})
...
def test_update_group_description_and_tags_description_changed(identity_client):
    group = create_group_object()
    identity_client.update_group.return_value = get_response(
        200, None, group, None)
...
def test_update_group_description_and_tags_defined_tags_changed(identity_client):
    ...
    identity_client.update_group.return_value = get_response(
        200, None, group, None)
    _, changed = oci_group.update_group_description_and_tags(
        identity_client, group, get_module(dict({'defined_tags': {'organisation': {'division': 'engineering'}}})))
    assert changed is True
...
def test_update_group_description_and_tags_defined_tags_not_changed(identity_client):
    ...
    identity_client.update_group.return_value = get_response(
        200, None, group, None)
    _, changed = oci_group.update_group_description_and_tags(
        identity_client, group, get_module(dict({'defined_tags': {'organisation': {'division': 'finance'}}})))
    assert changed is False
...
def test_update_group_description_and_tags_freeform_tags_changed(identity_client):
    ...
    identity_client.update_group.return_value = get_response(
        200, None, group, None)
    _, changed = oci_group.update_group_description_and_tags(
        identity_client, group, get_module(dict({'freeform_tags': {'member_type': 'developer'}})))
    assert changed is True
...
def test_update_group_description_and_tags_freeform_tags_not_changed(identity_client):
    ...
    identity_client.update_group.return_value = get_response(
        200, None, group, None)
    _, changed = oci_group.update_group_description_and_tags(
        identity_client, group, get_module(dict({'freeform_tags': {'member_type': 'CA'}})))
    assert changed is False
...
def test_update_group_users_new_user_success(mocker, identity_client):
    ...
    mock_add_users_to_group = mocker.patch.object(
        oci_group, 'add_users_to_group')
    existing_group_members = ['ocid1.testuser1']
    changed = oci_group.update_group_users(
        identity_client, 'test_compartment', None, existing_group_members, create_group_object(), get_module(dditional_properties))
...
def test_update_group_users_no_update(mocker, identity_client):
    ...
    changed = oci_group.update_group_users(
        identity_client, 'test_compartment', None, existing_group_members, create_group_object(), get_module(additional_properties))
...
def test_update_group_users_no_purge(mocker, identity_client):
    ...
    existing_group_members = ['ocid1.testuser1']
    changed = oci_group.update_group_users(
        identity_client, None, 'test_compartment', existing_group_members, create_group_object(), get_module(additional_properties))

    assert changed is False


def test_get_user_ids_from_user_names_no_matching_user_failure(identity_client, list_all_resources_patch):
    ...
    try:
        oci_group.get_user_ids_from_user_names(identity_client, [
            'ansible_user', 'ansible_user_two'], get_module(dict()))
    except Exception as ex:
        assert error_message in ex.args[0]


def test_get_user_ids_from_user_names_matching_user(identity_client, list_all_resources_patch):
    ansible_user = get_user()
    list_all_resources_patch.return_value = [ansible_user]
    result = oci_group.get_user_ids_from_user_names(identity_client, [
        'ansible_user'], get_module(dict({'compartment_id': 'test_compartment'})))
...
def test_delete_all_users_from_group_no_change(identity_client, list_all_resources_patch):
...
def test_get_difference_from_existing_users_no_existing_users():
    ...
    result = oci_group.get_difference_from_existing_users(
        existing_users, given_users)
...
def test_delete_group_success_no_force(mocker, identity_client, get_existing_resource_patch):
    additional_properties = dict(
    ...
    result = oci_group.delete_group(
        identity_client, get_module(additional_properties))
    assert result['changed'] is True


def test_delete_group_failure_no_force(mocker, identity_client, get_existing_resource_patch):
    ...
    identity_client.delete_group.side_effect = ServiceError(
        400, 'GroupNotEmpty', dict(), error_message)
    try:
        oci_group.delete_group(
            identity_client, get_module(additional_properties))
    except Exception as ex:
        assert error_message in ex.args[0]


def test_delete_group_success_force(mocker, identity_client, get_existing_resource_patch):
    additional_properties = dict(
        {'state': 'absent', 'force': 'no', 'compartment_id': 'compartment_id'})
...
def test_delete_group_failure_timeout_error(mocker, identity_client, get_existing_resource_patch):
