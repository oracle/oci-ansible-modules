...
def test_create_or_update_user_failure_service_error(identity_client, check_and_create_resource_patch):
...
def test_create_or_update_user_failure_timeout_error(identity_client, check_and_create_resource_patch):
...
def test_create_user_success(identity_client, create_and_wait_patch):
    ...
    create_and_wait_patch.return_value = dict(
        {'user': to_dict(user), 'changed': True})
    result = oci_user.create_user(identity_client, get_module(dict({
        'create_or_reset_ui_password': False, 'user_groups': None, 'compartment_id': 'test_compartment'})))
    assert result['user']['name'] is user.name


def test_create_user_with_group_success(identity_client,
                                        get_ids_from_group_names, add_user_to_groups, create_and_wait_patch):
    user = get_user()
    create_and_wait_patch.return_value = dict(
        {'user': to_dict(user), 'changed': True})
    ...
    result = oci_user.create_user(identity_client, get_module(dict({
        'create_or_reset_ui_password': False, 'user_groups': ['admin', 'network'], 'compartment_id': 'test_compartment'})))
    assert result['user']['name'] is user.name


def test_create_user_with_password_success(identity_client,
                                           create_or_reset_password, create_and_wait_patch):
    user = get_user()
    create_and_wait_patch.return_value = dict(
        {'user': to_dict(user), 'changed': True})
    create_or_reset_password.return_value = 'asd_09g'
    result = oci_user.create_user(identity_client, get_module(dict({
        'create_or_reset_ui_password': True, 'user_groups': None, 'compartment_id': 'test_compartment'})))
    assert result['user']['password'] is 'asd_09g'


def test_create_user_with_password_failure(identity_client,
                                           create_or_reset_password, create_and_wait_patch, delete_user_patch):
    error_message = 'Error during password generation'
    user = get_user()
    create_and_wait_patch = dict(
        {'user': to_dict(user), 'changed': True})
    create_or_reset_password.side_effect = ServiceError(
        499, 'InternalServerError', dict(), error_message)
    try:
        oci_user.create_user(identity_client, get_module(dict({
            'create_or_reset_ui_password': True, 'user_groups': None, 'compartment_id': 'test_compartment'})))
    except Exception as ex:
        assert error_message in ex.args[0]


def test_create_user_with_group_failure_no_group_exists(identity_client,
                                                        get_ids_from_group_names, add_user_to_groups, create_and_wait_patch, delete_user_patch):
...
def test_update_user_success(identity_client, update_user_description_and_tags,
                             modify_group_memberships, reset_ui_password, unblock_user):
    user = get_user()
    ...
    module = get_module(dict({
        'create_or_reset_ui_password': True, 'user_groups': ['admin', 'network'], 'blocked': 'no', 'compartment_id': 'test_compartment'}))
    changed, result = oci_user.update_user(
        identity_client, user, module)
    assert result['name'] is 'ansible_user'
    assert result['password'] is 'asd_09g'


def test_unblock_user_unblocked(identity_client):
    user = get_user()
    identity_client.update_user_state.return_value = get_response(
        200, None, user, None)
...
def test_reset_ui_password_changed(identity_client, create_or_reset_password):
    ...
    result, changed = oci_user.reset_ui_password(
        identity_client, get_user(), 'always')
...
def test_reset_ui_password_not_changed(identity_client):
    result, changed = oci_user.reset_ui_password(
        identity_client, get_user(), False)
    assert changed is False


def test_modify_group_memberships_group_updated(get_memberships, update_group_memberships):
    ...
    result = oci_user.modify_group_memberships(
        identity_client, ['test_group_1', 'test_group_2'], get_user(), get_module(dict({'compartment_id': 'test_compartment'})))
    assert result is True
...
def test_modify_group_memberships_group_removed(get_memberships, delete_memberships):
    ...
    result = oci_user.modify_group_memberships(
        identity_client, [], get_user(), get_module(dict({'compartment_id': 'test_compartment'})))
    assert result is True


def test_modify_group_memberships_no_changes():
    result = oci_user.modify_group_memberships(
        identity_client, None, get_user(), get_module(dict({'compartment_id': 'test_compartment'})))
    assert result is False
...
def test_update_group_memberships_changed(identity_client, get_ids_from_group_names):
    ...
    get_ids_from_group_names.return_value = [
        'ocid1.group.oc1..aaa', 'ocid1.group.oc1..xdv']
    result = oci_user.update_group_memberships(identity_client, 'test_compartment',
                                               ['test_group_1', 'test_group_2'],
                                               existing_memberships_group_ids,
                                               get_user().id, get_module(dict({'purge_group_memberships': False})))
    assert result is True


def test_update_group_memberships_not_changed(identity_client, get_ids_from_group_names):
    existing_memberships_group_ids = [
        'ocid1.group.oc1..aaa', 'ocid1.group.oc1..xdv']
    get_ids_from_group_names.return_value = [
        'ocid1.group.oc1..aaa', 'ocid1.group.oc1..xdv']
...
def test_update_group_memberships_purge(identity_client, get_ids_from_group_names, delete_memberships):
    existing_memberships_group_ids = ['ocid1.group.oc1..aaa']
    ...
    result = oci_user.update_group_memberships(identity_client, 'test_compartment',
                                               ['test_group_1'],
                                               existing_memberships_group_ids,
                                               get_user().id, get_module(dict({'purge_group_memberships': True})))
    assert result is True
...
def test_get_group_ids_from_group_names_matched_name(identity_client, list_all_resources_patch):
    ...
    result = oci_user.get_group_ids_from_group_names(identity_client,
                                                     ['test_group_one'], get_module(dict({'compartment_id': 'test_compartment'})))
    assert result[0] is 'ocid.test_group.1234xt2u'


def test_get_group_ids_from_group_names_no_matching_group_failure(identity_client, list_all_resources_patch):
    error_message = "Group test_group_three does not exist"
    group_one = create_group_object(
        'test_group_one', 'ocid.test_group.1234xt2u', 'Test Group One')
    list_all_resources_patch.return_value = [group_one]
    try:
        oci_user.get_group_ids_from_group_names(identity_client,
                                                ['test_group_three'], get_module(dict({'compartment_id': 'test_compartment'})))
    except Exception as ex:
        assert error_message in ex.args[0]


def test_delete_all_group_memberships_existing_memberships(identity_client, list_all_resources_patch):
...
def test_delete_all_group_memberships_no_memberships(identity_client, list_all_resources_patch):
...
def test_add_user_to_groups(identity_client):
    ...
    oci_user.add_user_to_groups(identity_client, 'ocid1.user.oc1..aaa', [
                                'ocid1.group.oc1..xdv'])


def test_get_group_ids_from_existing_memberships(identity_client, list_all_resources_patch):
    user_group_membership_one = UserGroupMembership()
...
def test_delete_user_no_memberships(identity_client, get_existing_resource_patch, delete_and_wait_patch):
    user = get_user()
    get_existing_resource_patch.return_value = user
    delete_and_wait_patch.return_value = {
        'user': to_dict(user), 'changed': True}
    module = get_module(dict(
        {'force': False, 'compartment_id': 'ocid1.compartment..abuw', 'user_id': 'ocid1.user..abuw'}))
    result = oci_user.delete_user(identity_client, module)
    assert result['changed'] is True


def test_delete_user_with_memberships_force_yes(identity_client, get_existing_resource_patch, delete_memberships, delete_and_wait_patch):
    user = get_user()
    get_existing_resource_patch.return_value = user
    delete_and_wait_patch.return_value = {
        'user': to_dict(user), 'changed': True}
    delete_memberships.return_value = None
    module = get_module(dict(
        {'force': True, 'compartment_id': 'ocid1.compartment..abuw', 'user_id': 'ocid1.user..abuw'}))
    result = oci_user.delete_user(identity_client, module)
    assert result['changed'] is True


def test_delete_user_with_memberships_force_no_failure(identity_client, get_existing_resource_patch, delete_memberships):
